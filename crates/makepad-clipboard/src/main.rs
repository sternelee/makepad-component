    ...
    startup() do #(App::script_component(vm)){
        ui: Root{
            main_window := Window{
                ...
                window.inner_size: vec2(800, 700)
                show_bg: true
                draw_bg +: {
                    pixel: fn() {
                ...
                body := View{
                    ...
                    buttons := View{
                        ...
                        btn_refresh := Button{ text: "🔄 Refresh", width: Fit, height: Fit, padding: Inset{left: 12, right: 12, top: 6, bottom: 6} }
                        btn_auto := Button{ text: "Auto: OFF", width: Fit, height: Fit, padding: Inset{left: 12, right: 12, top: 6, bottom: 6} }
                        btn_clear := Button{ text: "🗑 Clear", width: Fit, height: Fit, padding: Inset{left: 12, right: 12, top: 6, bottom: 6} }
                        lbl_status := Label{
                            width: Fill
                            height: Fit
                            text: "Ready"
                    ...
                    img_info := Label{
                        ...
                        padding: Inset{left: 0, right: 8, top: 0, bottom: 8}
                        text: ""
                        draw_text +: {
                    ...
                    latest_txt := Label{
                        width: Fill
                        height: Fit
                        text: "No entries yet. Click Refresh!"
                        draw_text +: {
...
impl AppMain for App {
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event) {
        match event {
            ...
            Event::KeyDown(ke) => {
                ...
                if ke.key_code == KeyCode::Escape {
                    ...
                    self.ui.button(cx, ids!(btn_auto)).set_text(cx, "Auto: OFF");
                }
            }
            _ => {}
        }
        ...
        if self.ui.button(cx, ids!(btn_refresh)).clicked(&actions) {
            self.poll_clipboard(cx);
        }
        if self.ui.button(cx, ids!(btn_auto)).clicked(&actions) {
            self.auto_refresh = !self.auto_refresh;
            self.ui.button(cx, ids!(btn_auto)).set_text(
                cx,
                if self.auto_refresh {
                    "⏸ Auto: ON"
                } else {
        ...
        if self.ui.button(cx, ids!(btn_clear)).clicked(&actions) {
            self.clear_all(cx);
        }
    }
}
...
impl App {
    ...
    fn clear_all(&mut self, cx: &mut Cx) {
        ...
        self.ui.label(cx, ids!(lbl_status)).set_text(cx, "Cleared");
        self.ui
            .label(cx, ids!(latest_txt))
            .set_text(cx, "No entries yet. Click Refresh!");
        self.ui.view(cx, ids!(img_box)).set_visible(cx, false);
        self.ui.image(cx, ids!(img_preview)).set_visible(cx, false);
        self.ui.label(cx, ids!(img_info)).set_visible(cx, false);
        self.ui.label(cx, ids!(history_list)).set_text(cx, "");
    }
        ...
        if let Some(latest) = entries.first() {
            match &latest.content {
                makepad_clipboard::ClipboardContent::Image {
                    ...
                    self.ui.label(cx, ids!(latest_txt)).set_visible(cx, false);
                    self.ui.view(cx, ids!(img_box)).set_visible(cx, true);
                    self.ui.image(cx, ids!(img_preview)).set_visible(cx, true);
                    self.ui.label(cx, ids!(img_info)).set_visible(cx, true);
                    self.ui
                        .label(cx, ids!(img_info))
                    ...
                    if !data.is_empty() {
                        ...
                        if fs::write(&path, data).is_ok() {
                            ...
                            let _ = self
                                .ui
                                .image(cx, ids!(img_preview))
                                .load_image_file_by_path(
                                    cx,
                                    std::path::Path::new(
                                        &self.current_image_path.as_ref().unwrap(),
                                    ),
                                );
                        }
                    }
                ...
                makepad_clipboard::ClipboardContent::Text(text) => {
                    self.ui.label(cx, ids!(latest_txt)).set_visible(cx, true);
                    self.ui.view(cx, ids!(img_box)).set_visible(cx, false);
                    self.ui.image(cx, ids!(img_preview)).set_visible(cx, false);
                    self.ui.label(cx, ids!(img_info)).set_visible(cx, false);
                    self.ui.label(cx, ids!(latest_txt)).set_text(cx, text);
                }
