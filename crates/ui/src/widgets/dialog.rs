...
script_mod! {
    ...
    // ============================================================
    // MpDialog - Animated modal dialog with shadcn/styling
    // Builds on top of the existing MpModal/MpModalWidget
    // ============================================================

    // Full-screen dialog (centered card with backdrop)
    ...
    mod.widgets.MpDialog = set_type_default() do mod.widgets.MpDialogBase{
        ...
        backdrop := View{
            ...
            show_bg: true
            draw_bg +: {
                bg_color: instance(#x000000)
                opacity: instance(0.0)
                pixel: fn() {
                    return self.bg_color * self.opacity
                }
            }
        }
        ...
        // Animator for backdrop fade + dialog pop
        animator: Animator{
            show: {
                default: @off
                off: AnimatorState{
    ...
    mod.widgets.MpDialogWide = mod.widgets.MpDialogInner{
        width: 640
        max_height: 700.0
    }
...
impl MpDialog {
    ...
    pub fn set_open(&mut self, cx: &mut Cx, open: bool) {
        if open { self.open(cx); } else { self.close(cx); }
    }

    pub fn is_open(&self) -> bool { self.open }

    pub fn set_title(&mut self, cx: &mut Cx, title: &str) {
        self.view.label(cx, ids!(content.dialog.header.title)).set_text(cx, title);
    }

    pub fn set_description(&mut self, cx: &mut Cx, desc: &str) {
        let desc_label = self.view.label(cx, ids!(content.dialog.header.description));
        desc_label.set_text(cx, desc);
        self.view.view(cx, ids!(content.dialog.header.description)).set_visible(cx, !desc.is_empty());
    }
}
...
impl MpDialogRef {
    ...
    pub fn set_open(&self, cx: &mut Cx, open: bool) {
        if let Some(mut inner) = self.borrow_mut() { inner.set_open(cx, open); }
    }
    pub fn is_open(&self) -> bool {
        if let Some(inner) = self.borrow() { inner.is_open() } else { false }
    }
    pub fn set_title(&self, cx: &mut Cx, title: &str) {
        if let Some(mut inner) = self.borrow_mut() { inner.set_title(cx, title); }
    }
    pub fn set_description(&self, cx: &mut Cx, desc: &str) {
        if let Some(mut inner) = self.borrow_mut() { inner.set_description(cx, desc); }
    }
